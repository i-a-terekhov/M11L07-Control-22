...
import {
    ...
    ResponseOfCategoryResult, ResponseOfOperationResult
} from "../../types/response-of-http-request";
import {PageHandler} from "../page-handler";

export class CreateNewOperation extends PageHandler {
    ...
    constructor(openNewRouteFunction: Function) {
        ...
        if (window.location.search && window.location.search.split('=')[1]) {
            let typeOfOperation: string = decodeURIComponent((window.location.search.split('=')[1]) as string);
            if (typeOfOperation) {
    ...
    private async pushDataToServer(): Promise<boolean> {
        ...
        if (allCategories) {
            ...
            let response: ResponseOfOperationResult = await OperationsService.createNewOperation(body);
            return true
